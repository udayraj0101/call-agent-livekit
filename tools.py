...
DEMO MODE: both tools are commented out. With an empty tools manifest the LLM
skips the "should I call a function?" deliberation on every turn, saving
~200-400ms per turn. Caller hangs up manually instead of relying on end_call.

Restore for production: uncomment the function definitions and add them back
to the create_tools() return list.
"""
import asyncio
import logging
import os
...
def create_tools(ctx: agents.JobContext):
    # === DEMO MODE: no tools (latency win ~200-400ms per turn) ===
    # The caller hangs up manually instead of the agent invoking end_call.
    # See the COMMENTED-OUT function definitions below — uncomment and add
    # back to the return list to restore production behaviour.
    return []

    # ------------------------------------------------------------------
    # COMMENTED OUT FOR DEMO — restore by un-commenting and including in
    # the return list above.
    # ------------------------------------------------------------------
    ...
    #     logger.info("Agent requested call end — deleting room in 8s.")
    #
